...
tf.flags.DEFINE_string("dir", "../data/yelp", "The directory of the dataset")
...
tf.flags.DEFINE_string("para_data", "data_2.para", "Data parameters")
...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            # optimizer = tf.train.AdamOptimizer(0.002, beta1=0.9, beta2=0.999, epsilon=1e-8).minimize(deep.loss) # AdadeltaOptimizer # AdagradOptimizer # 
            optimizer = tf.train.AdagradOptimizer(0.005).minimize(deep.loss)
            ...
            # pkl_file.close()
            train_data = np.loadtxt(os.path.join(FLAGS.dir, 'train.csv'), delimiter=',', dtype=int)
            train_data = train_data[:, :, np.newaxis]

            test_data = np.loadtxt(os.path.join(FLAGS.dir, 'test.csv'), delimiter=',', dtype=int)
            test_data = test_data[:, :, np.newaxis]
            # train_data = np.loadtxt(os.path.join(FLAGS.dir, 'train.csv'), delimiter=',', dtype=int).tolist()
            ...
            for epoch in range(FLAGS.num_epochs):
                ...
                for batch_num in range(ll):
                    ...
                    t_rmse, t_mae, u_a, i_a = train_step(u_f_batch, i_f_batch, uid, iid, reuid, reiid, y_batch,
                                                             batch_num)
                    current_step = tf.train.global_step(sess, global_step)
                    train_rmse += t_rmse
                    train_mae += t_mae
                    if batch_num % 5000 == 0 and batch_num > 1:
                        print("\nEvaluation:")
                        print(batch_num)
