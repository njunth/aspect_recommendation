...
tf.flags.DEFINE_string("model", "APNCF", "model to train")
# ==================================================
...
def train_step(u_batch, i_batch, uid, iid, reuid, reiid, y_batch, batch_num):
    ...
    u_a, i_a = 0, 0
    # print(sess.run([deep.u_feas1, deep.i_feas1, deep.input_u_feature], feed_dict))
    # exit(-1)
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            for uid in all_uid:
                # print(uid)
                # uid = uid[0]
                user_features_dict[uid] = build_user_preference2(user_features[uid])
            for iid in all_iid:
                # print(iid)
                # iid = iid[0]
                item_features_dict[iid] = build_item_characteristics(item_features[iid])
